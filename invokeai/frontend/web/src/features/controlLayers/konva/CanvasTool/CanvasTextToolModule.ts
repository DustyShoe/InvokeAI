...
export class CanvasTextToolModule extends CanvasModuleBase {
  ...
  setClipboardHandlers = (clipboardHandlers: ClipboardHandlers) => {
    this.clipboardHandlers = clipboardHandlers;
  };

  clearClipboardHandlers = () => {
    this.clipboardHandlers = null;
  };
  ...
  private copyText = (text: string) => {
    if (!text || !this.clipboardHandlers) {
      return;
    }

    try {
      this.clipboardHandlers.writeText(text);
    } catch (error) {
  ...
  private pasteFromClipboard = async () => {
    if (!this.clipboardHandlers) {
      return;
    }

    try {
      const clipboardText = await this.clipboardHandlers.readText();
      if (!clipboardText) {
        return;
      }
